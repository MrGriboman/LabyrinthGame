...
public:
	...
	void money(int counter) {
		srand(time(NULL));
		while (counter > 0) {
			int pointer_1 = rand() % const_x;
			int pointer_2 = rand() % const_y;
			if (lab[pointer_1][pointer_2] != 0) {
				lab[pointer_1][pointer_2] = 2;
				counter--;
			}
		}
	}
	...
	std::wstring draw() { //draws a maze
		std::wstring maze = L"";
		for (int i = 0; i < const_y; i++) {
			for (int j = 0; j < const_x; j++) {
				if ((i == end_y + 1 && j == end_x))
					maze += L" "; //exit
				else if (lab[i][j] == 0)
					maze += L"#"; //wall
				else if (lab[i][j] == 2)
